...
The performance of different models are summarized in Table \ref{eval_table}. A unidirectional GRU model without attention is used for establishing the baseline for a sequential RS. All the BST and NGCF models are implemented as described in Section \ref{baseline:bst} and Section \ref{baseline:ngcf} respectively. Different sets of sequence lengths were analysed for the baseline BST model. Then, we implement the proposed TransRecG model as described in Section \ref{proposed}. We also analyse the performance of the NGCF (with RGCN) model on the KG without a BST, and the performance of the BST with node embeddings from NGCF on the user-movie bipartite graph. 
